...
exports.makePack = functions.https.onRequest(async (req, res) => {
	...
	archive.append(moduleSelection(format, modules, iconModules, optionsBackground, panoOption), {
		name: 'modules.txt'
	}); // add modules.txt file
	...
	output.on('close', async () => {
		console.log(
			'Archiver has been finalized and the output file descriptor has closed. File size: ' +
				archive.pointer() +
				' bytes'
		);
...
function mcMeta(format: string) {
    ...
    "pack": {
		...
		packFormat +
		`,
        "description": "§aFaithful Tweaks §6- §c` +
		formatStr +
