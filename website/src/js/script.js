...
window.addEventListener('DOMContentLoaded', () => {
	...
	document
		...
		.forEach((e) => e.addEventListener('click', setModuleConflicting)); // Conflicting normal modules

	// HUD
	document
		.querySelectorAll('#hudModules                 >*')
		.forEach((e) => e.addEventListener('click', setIconModule)); // Normal modules
	document
		.querySelectorAll('#hudModulesConflicting      >*')
		.forEach((e) => e.addEventListener('click', setIconModuleConflicting)); // Conflicting normal modules

	// Options and Panoramas
	document
		.querySelectorAll('#optionsbgModules           >*')
		.forEach((e) => e.addEventListener('click', setBackground)); // Backgrounds
...
function setModuleConflicting() {
	...
	Array.prototype.filter
		.call(this.parentNode.children, (c) => {
			return c !== this;
		})
		.forEach((e) => {
...
function setBackground() {
	...
	Array.prototype.filter
		...
		.forEach((e) => e.classList.remove('enabled'));
...
function downloadPack() {
	...
	const url =
		process.env.NODE_ENV !== 'production'
			? 'http://localhost:5001/faithfultweaks/us-central1/makePack'
			: 'https://us-central1-vanilla-extract.cloudfunctions.net/makePack'; // URL (based on node environment status)
	const data = {
		format: version,
		modules: modules,
